...
class TensorRTReranker(BaseTensorrtInferencer):
    ...
    def build_cuda_graph(self, batch_size: int, max_length: int = None):
        ...
        # dummy data
        pair = ("hello world", "goodbye world")
        pairs = [pair] * batch_size

        enc = self.tokenizer(pairs,
                             return_tensors="np",
    ...
    def infer(self, pairs: list[tuple[str, str]], cuda_graph=True):
        ...
        scores = self.buffers_host[self.output_names[0]][:orig_n].tolist()
        return scores, elapsed_ms